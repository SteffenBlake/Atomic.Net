    ...
    ISingleton<RulesDriver>
{
    ...
    private void ProcessRule(JsonRule rule, float deltaTime)
    {
        ...
        // Process commands for each filtered entity
    ...
    /// <summary>
    /// Processes a command on an entity (supports all SceneCommand types).
    /// </summary>
    private void ProcessEntityCommand(JsonNode entity, SceneCommand command, JsonObject ruleContext)
    {
        ...
        // Handle SequenceStartCommand
        if (command.TryMatch(out Sequencing.SequenceStartCommand seqStartCmd))
        {
            Sequencing.SequenceStartCmdDriver.Execute(entityIndex.Value, seqStartCmd.SequenceId);
            return;
        }

        // Handle SequenceStopCommand
        if (command.TryMatch(out Sequencing.SequenceStopCommand seqStopCmd))
        {
            Sequencing.SequenceStopCmdDriver.Execute(entityIndex.Value, seqStopCmd.SequenceId);
            return;
        }

        // Handle SequenceResetCommand
        if (command.TryMatch(out Sequencing.SequenceResetCommand seqResetCmd))
        {
            Sequencing.SequenceResetCmdDriver.Execute(entityIndex.Value, seqResetCmd.SequenceId);
            return;
        }

        EventBus<ErrorEvent>.Push(new ErrorEvent(
            $"Unrecognized command type for entity {entityIndex.Value}"
        ));
    }
