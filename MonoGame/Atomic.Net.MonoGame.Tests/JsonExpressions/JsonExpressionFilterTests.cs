...
/// </summary>
public sealed class JsonExpressionFilterTests : IDisposable
{
    ...
    public JsonExpressionFilterTests(ITestOutputHelper output)
    {
    ...
    {
        // Arrange
        var json = """{"filter": [{"var": "Numbers"}, {"%": [{"var": ""}, 2]}]}""";
    ...
    {
        // Arrange
        var json = """{"filter": [{"var": "Numbers"}, {">": [{"var": ""}, 0]}]}""";
        var doc = JsonDocument.Parse(json);
        var expr = JsonExpression.Compile<TestInput, int[]>(doc);
        var func = expr.Compile();
    ...
    {
        // Arrange
        var json = """{"filter": [{"var": "Numbers"}, {">": [{"var": ""}, 100]}]}""";
        var doc = JsonDocument.Parse(json);
        var expr = JsonExpression.Compile<TestInput, int[]>(doc);
        var func = expr.Compile();
        var data = new TestInput(new[] { 1, 2, 3, 4, 5 });
