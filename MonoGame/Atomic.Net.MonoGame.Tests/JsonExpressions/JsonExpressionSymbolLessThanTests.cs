...
public sealed class JsonExpressionSymbolLessThanTests(ITestOutputHelper output) : IDisposable
{
    ...
    [Fact]
    public void LessThan_NotAnArray_Fails()
    {
    ...
    [Fact]
    public void LessThan_TooFewArguments_Fails()
    {
        var json = """{"<": [1]}""";
        var doc = JsonDocument.Parse(json);
        Assert.False(JsonExpressionCompiler.TryBuild<TestInput, bool>(doc, out _));
