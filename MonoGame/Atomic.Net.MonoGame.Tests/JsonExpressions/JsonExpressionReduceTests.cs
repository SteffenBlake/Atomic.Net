...
/// </summary>
public sealed class JsonExpressionReduceTests : IDisposable
{
    ...
    public JsonExpressionReduceTests(ITestOutputHelper output)
    {
    ...
    {
        // Arrange
        var json = """{"reduce": [{"var": "Numbers"}, {"+": [{"var": "current"}, {"var": "accumulator"}]}, 0]}""";
    ...
    {
        // Arrange
        var json = """{"reduce": [{"var": "Numbers"}, {"*": [{"var": "current"}, {"var": "accumulator"}]}, 1]}""";
        var doc = JsonDocument.Parse(json);
        var expr = JsonExpression.Compile<TestInput, int>(doc);
        var func = expr.Compile();
    ...
    {
        // Arrange
        var json = """{"reduce": [{"var": "Numbers"}, {"+": [{"var": "current"}, {"var": "accumulator"}]}, 100]}""";
        var doc = JsonDocument.Parse(json);
        var expr = JsonExpression.Compile<TestInput, int>(doc);
        var func = expr.Compile();
    ...
    {
        // Arrange
        var json = """{"reduce": [{"var": "Numbers"}, {"if": [{">": [{"var": "current"}, {"var": "accumulator"}]}, {"var": "current"}, {"var": "accumulator"}]}, 0]}""";
        var doc = JsonDocument.Parse(json);
        var expr = JsonExpression.Compile<TestInput, int>(doc);
        var func = expr.Compile();
    ...
    {
        // Arrange
        var json = """{"reduce": [{"var": "Numbers"}, {"+": [{"var": "current"}, {"var": "accumulator"}]}, 50]}""";
        var doc = JsonDocument.Parse(json);
        var expr = JsonExpression.Compile<TestInput, int>(doc);
        var func = expr.Compile();
