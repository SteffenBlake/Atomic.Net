...
/// <summary>
/// Computes element-wise addition over one input block and a scalar: output[i] = input[i] + s.
/// </summary>
public sealed class AddScalarBlockMap(
    BlockMapBase input,
    FloatMapBase s,
...
) : UnaryAndScalarBlockMapBase(input, s, initValue, dense, blockSize)
{
    ...
    {
        TensorPrimitives.Add(inputBlock, s, outputBlock);
    }
}
